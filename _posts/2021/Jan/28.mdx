...
__if__ : No ':', but 'end' is needed.

__intmax(__ class __)__ : return the largest value(int) of the given class(e.g. int8, int16 etc.) 

__audioread(__ path, opt __)__ : read audio file from path, with opt='native' or 'double'. The latter option allows to normalize.

__NOTE__: the return value is of double only if denominator / numerator are double.

__which__: where is the function? It is used to check the function _resample_ is what I wanted.
...
* 1000 - 1050: PCG envelograms and Springer's method 마저 구현하기..! 오늘안에 할 수 있을까?
...
* 1628 - 1715: Reidemeister Torsion!

* 1715 - 1901: 태진이형이랑 수다떨다가 knot 계산 살펴보기(2963이랑 2693을 헷갈렸다..)
